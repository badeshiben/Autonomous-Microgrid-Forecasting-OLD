...
split_point = len(series) - 408 ## To start from 2472 datapoint
dataset, validation = series[0:split_point], series[split_point:2652]
input1 =series[2652:]
...
rmse = sqrt(mean_squared_error(y[15:], predictions[15:]))

print('RMSE: %.3f' % rmse)
pyplot.plot(y[15:])
pyplot.plot(predictions[15:], color='red')
pyplot.show()
...
# removed last 24 datapoints for final validation towards the end.
split_point = len(series) - 408 ## To start from 2001 datapoint
dataset, validation = series[0:split_point], series[split_point:2652]
input1 =series[2652:]
#dataset.extend(input1) 
#dataset = numpy.append (dataset, input1)
