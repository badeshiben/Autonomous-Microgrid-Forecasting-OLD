...
series = read_csv(r'C:\Users\rkhan\Documents\MIRACL\ARIMA_Forecasting_Python\Time_Series\WS_TV\WSTV1.csv',
...
split_point = len(series) - 1230 ## To start from 1650 datapoint
dataset, validation = series[0:split_point], series[split_point:1830]
input1 =series[1830:]
...
model = ARIMA(series, order=(5, 1, 4)).fit()
#model_fit = model.fit()
...
for t in range(len(test)):
    model = ARIMA(history,  order=(5, 1, 4)).fit()
...
def relative_root_mean_squared_error(true, pred):
    num =sum(numpy.square(true - pred))
    den = sum(numpy.square(pred))
...
RRMSE = relative_root_mean_squared_error(y, predictions)
print(RRMSE)
